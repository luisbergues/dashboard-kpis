...
// --- Suggestions (starter chips on open + typeahead while typing) ---

// A suggestion is what the user sees and what gets sent when they click it:
//   { type, label, sub, query }
// `query` is fed back through the normal processInput path, so a suggestion is
// just a pre-filled question — nothing about resolving it is special-cased.

function makeProjectSuggestion(p) {
  return { type: 'project', label: String(p.name || `SO ${p.so}`), sub: `SO #${p.so}`, query: String(p.so) };
}
function makePersonSuggestion(name) {
  return { type: 'person', label: name, sub: '', query: name };
}
function makeContactSuggestion(c) {
  // "contacto" is an intent stopword, so this still resolves to the contact
  // card while signalling intent if the person is also a designer with projects.
  return { type: 'contact', label: c.name, sub: 'Contacto', query: `${c.name} contacto` };
}

// Randomly samples n items from arr without replacement. rng is injectable so
...
// Starter suggestions shown when the chat opens with no history: a random mix
// of real projects and people so the user discovers they can ask about
// specific things. Random so different items surface each time — aiding
// discovery rather than always showing the same few.
export function buildRandomSuggestions({ projects = [], designerContacts = [], count = 4, rng = Math.random } = {}) {
  const items = [];

  sampleN(projects, 2, rng).forEach(p => items.push(makeProjectSuggestion(p)));

  const people = [...new Set(projects.flatMap(p => [p.designer, p.eng]).filter(Boolean))];
  sampleN(people, 1, rng).forEach(name => items.push(makePersonSuggestion(name)));

  sampleN(designerContacts, 1, rng).forEach(c => items.push(makeContactSuggestion(c)));

  // If some categories were empty, backfill from whatever's left so the row
  // still feels populated (up to `count`).
  if (items.length < count) {
    const usedSo = new Set(items.filter(i => i.type === 'project').map(i => i.query));
    sampleN(projects.filter(p => !usedSo.has(String(p.so))), count - items.length, rng)
      .forEach(p => items.push(makeProjectSuggestion(p)));
  }

  return items.slice(0, count);
}

// Typeahead suggestions for what the user is typing. Deliberately more
// permissive than findLocalEntityMatches: prefix on SO, substring on names,
// with no 4-char floor — a half-typed "k8" should still surface "K800", where
// reach matters more than the precision a complete query needs. Ranked so
// prefix/exact hits sit above mid-string ones.
export function buildTypeaheadSuggestions(text, { projects = [], designerContacts = [], limit = 6 } = {}) {
  const q = String(text || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');
  if (q.length < 2) return [];

  const seen = new Set();
  const scored = [];
  const add = (item, score) => {
    if (seen.has(item.query + item.type)) return;
    seen.add(item.query + item.type);
    scored.push({ item, score });
  };

  projects.forEach(p => {
    const so = String(p.so || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const name = String(p.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    if (so && so.startsWith(q)) add(makeProjectSuggestion(p), 3);
    else if (name && name.startsWith(q)) add(makeProjectSuggestion(p), 2);
    else if ((so && so.includes(q)) || (name && name.includes(q))) add(makeProjectSuggestion(p), 1);
  });

  const people = [...new Set(projects.flatMap(p => [p.designer, p.eng]).filter(Boolean))];
  people.forEach(name => {
    const n = String(name).toLowerCase().replace(/[^a-z0-9]/g, '');
    if (n.startsWith(q)) add(makePersonSuggestion(name), 2);
    else if (n.includes(q)) add(makePersonSuggestion(name), 1);
  });

  designerContacts.forEach(c => {
    const n = String(c.name || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    if (n.startsWith(q)) add(makeContactSuggestion(c), 2);
    else if (n.includes(q)) add(makeContactSuggestion(c), 1);
  });
