...
import { calculateCADErrors } from '../services/kpiCalculator';
import SectionErrorBoundary from '../components/SectionErrorBoundary';

ChartJS.register(CategoryScale, LinearScale, BarElement, Title, ChartTooltip, Legend);

export default function DesignQualityView({ data, filters }) {
  const { t } = useLanguage();

  const { onHoldNotes = [] } = data || {};

  const { errorCounts, totalCADErrors } = useMemo(() => calculateCADErrors(onHoldNotes), [onHoldNotes]);

  const chartData = useMemo(() => {
    // Sort designers by error count
    const sortedEntries = Object.entries(errorCounts).sort((a, b) => b[1] - a[1]);
    const labels = sortedEntries.map(e => e[0]);
    const datasetData = sortedEntries.map(e => e[1]);
  ...
  return (
    <div className="design-quality-view animate-fade-in" style={{ padding: '24px' }}>
      <header className="dashboard-header" style={{ marginBottom: '24px' }}>
        ...
        <p className="page-subtitle text-muted" style={{ color: '#94A3B8' }}>Tracking CAD errors and validation times</p>
      </header>
      ...
      <section className="kpi-metrics-row mb-xl" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
        <div className="glass-card kpi-card" style={{ padding: '20px', borderRadius: '12px', background: 'rgba(11, 21, 32, 0.6)', border: '1px solid rgba(255, 255, 255, 0.05)', display: 'flex', flexDirection: 'column' }}>
          <span className="kpi-label" style={{ color: '#94A3B8', fontSize: '0.85rem', marginBottom: '8px' }}>Total CAD Errors</span>
          <span className="kpi-value text-danger" style={{ fontSize: '1.5rem', fontWeight: 700, color: '#FF2E93' }}>{totalCADErrors}</span>
        </div>
      </section>
